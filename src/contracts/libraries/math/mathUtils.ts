...
export class MathUtils {
     ...
     */
    static calculateLinearInterest(rate: bigint, lastUpdateTimestamp: number): bigint {
        const timeDifference = BigInt(Date.now() / 1000) - BigInt(lastUpdateTimestamp);
     ...
     */
    static _calculateCompoundedInterest(rate: bigint, lastUpdateTimestamp: number, currentTimestamp: number): bigint {
        ...
        const thirdTerm = (exp * expMinusOne * expMinusTwo * basePowerThree) / BigInt(6);
     ...
     */
    static calculateCompoundedInterest(rate: bigint, lastUpdateTimestamp: number): bigint {
        return this._calculateCompoundedInterest(rate, lastUpdateTimestamp, Math.floor(Date.now() / 1000));
    }
}